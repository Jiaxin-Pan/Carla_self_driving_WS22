...
def compute_steering(image):
    #image.save_to_disk('_out/%06d.png' % frame, cc)
    ...
    img = np.repeat(logdepth[:, :, np.newaxis], 3, axis=2)
    img = img *255
    #global rgb_list
    #rgb_list.append(img/255)
    
    torch_img = torch.from_numpy(np.transpose(img, (2, 0, 1)))
    torch_img = torch_img.float()
    #cv2.imshow("image",img)
    #cv2.waitKey(10)
    #Z = dummy_model(torch_img)
...
def main():
    ...
    try:
        ...
        transform = world.get_map().get_spawn_points()[20]
        # pdb.set_trace()
        ...
        camera.listen(lambda image: compute_steering(image))
        vehicle.apply_control(carla.VehicleControl(throttle=0.5, steer=Z_list[-1]))   
