...
PATH = 'models/2extra_camera.p'
steer_model = SteerNN(None)
steer_model.load_state_dict(torch.load(PATH))
dummy_model = DummySteerModel()
image_list = np.zeros([251,3,600,800])
Z_list = [0]
rgb_list = list()

def logistic_depth(image):
    #convert from raw_data to logistic map
    ...
    img = np.repeat(logdepth[:, :, np.newaxis], 1, axis=2)
    img = img *255
    img = np.transpose(img, (2, 0, 1))    #shape:(1,600,800)
...
def save_to_list(image,frame,p):
    print("here", frame)
    img = logistic_depth(image)
    global image_list
    image_list[frame,p,:,:] = img

    #vehicle.apply_control(carla.VehicleControl(throttle=1.0, steer=Z))
    #print("Z_list" ,Z_list)
    #return Z
...
def main():
    ...
    try:
        ...
        # Add two more "depth" cameras on right and left sides
        camera_bp_left = blueprint_library.find('sensor.camera.depth')
        #camera_transform_left = carla.Transform(carla.Location(x=1.5, y =-4, z=2.4))
        camera_transform_left = carla.Transform(carla.Location(x=1.5, y=-2, z=2.4), carla.Rotation(pitch=0,yaw=10,roll=0))
        camera_left = world.spawn_actor(camera_bp_left, camera_transform_left, attach_to=vehicle)
        actor_list.append(camera_left)
        print('created %s' % camera_left.type_id)

        camera_bp_lright = blueprint_library.find('sensor.camera.depth')
        camera_transform_right = carla.Transform(carla.Location(x=1.5, y=2, z=2.4), carla.Rotation(pitch=0,yaw=-10,roll=0))
        ...
        camera.listen(lambda image: save_to_list(image,frame,1))
        camera_left.listen(lambda image: save_to_list(image,frame,0))
        camera_right.listen(lambda image: save_to_list(image,frame,2))
        vehicle.apply_control(carla.VehicleControl(throttle=0.3, steer=Z_list[-1]))   
        ...
        for frame in range(251):
            ...
            global image_list
            frame_img = image_list[frame,:,:,:]  #shape:(3,600,800)
            torch_img = torch.from_numpy(frame_img)
            torch_img = torch_img.float()
            #compute the corresponding steer
            Z_tensor = steer_model(torch_img)/100
            Z = float(Z_tensor)
            #apply control
            vehicle.apply_control(carla.VehicleControl(throttle=0.3, steer=Z))
            if frame>10:
                #the color is opposite as the one in notebook, since cv2 gives BGR and plt RGB
                visualize_image((np.transpose(image_list[frame,:,:,:], (1, 2, 0)))/255) 
